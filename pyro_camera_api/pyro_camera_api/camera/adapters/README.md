...
| ---------------------------------- | -------------------- | ------- | --- | ----- | ------------------------------------------- |
| `reolink-823S2`, `reolink-823A16`  | `ReolinkCamera`      | yes     | yes | yes   | `ip_address`; PTZ uses `poses`, `azimuths`  |
...
A new adapter is justified when the camera has a proprietary protocol or when a stable family
integration removes repeated, secret-bearing configuration from deployments. It should still
...
Streaming is configured separately from camera adapters. The optional `rtsp_path` overrides the
default path generated by `core/config.py`. Its value depends on the camera firmware and must be
confirmed before use. For example, a CTronics camera may require a path such as `/11`, but this
is not guaranteed across models.
...
- **Streaming is outside the adapter scope, at this stage.** It is built separately in
  `core/config.py`: stream URLs are assembled from the camera IP, assuming RTSP on port 554,
  with a Reolink-like path by default or a Linovision channel path. An adapter therefore has
  nothing to implement for streaming, but it also has no say in the generated URL. A camera that
  does not follow those conventions can capture correctly yet still fail to stream; `rtsp_path`
  provides a configuration override for the path, but not for the complete URL construction.
  Moving URL construction down into the adapters would be consistent with the rest of the design;
  it is not done today and is worth keeping in mind before integrating a camera whose stream does
  not look like `rtsp://<ip>:554/...`.
