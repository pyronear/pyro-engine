...
CAPTURE_TIMEOUT_S = 5.0  # hard timeout for a single capture call


# ----------------------------------------------------------------------
# Helper to call cam.capture with a timeout
# ----------------------------------------------------------------------

def capture_with_timeout(cam, timeout_s: float):
    """
    Run cam.capture() in a worker thread, wait at most timeout_s seconds.
    Returns the image or None on timeout, re raises any exception from capture.
    """
    result = {"image": None, "error": None}

    def _worker():
        try:
            result["image"] = cam.capture()
    ...
    if t.is_alive():
        logging.error("Capture timeout after %.1fs for camera %s", timeout_s, getattr(cam, "ip_address", "unknown"))
        return None

    if result["error"] is not None:
        # propagate the original exception so your existing try except sees it
        raise result["error"]
...
def static_loop(camera_ip: str, stop_flag: threading.Event):
    ...
    while not stop_flag.is_set():
        try:
            # use the timeout wrapper instead of calling cam.capture directly
            image = capture_with_timeout(cam, CAPTURE_TIMEOUT_S)
            now = time.time()
