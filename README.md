...
* `ctronics` adapter, for CTronics cameras exposing an authenticated HTTP snapshot endpoint
...
For CTronics, use `adapter: "ctronics"` with `ip_address` (or the configuration key), optional `username`/`password`, and optionally `port`, `protocol`, `snapshot_path`, `snapshot_command`, `model`, and `rtsp_path`. The tested default snapshot endpoint is `/tmpfs/snap.jpg?usr=...&pwd=...`; verify the endpoint on other models before deployment.
