...
* `ctronics` adapter, for CTronics cameras exposing the CGI snapshot endpoint
...
For CTronics, use `adapter: "ctronics"` with `ip_address` (or the configuration key), optional `username`/`password`, and optionally `port`, `protocol`, `snapshot_path`, `snapshot_command`, `model`, and `rtsp_path`. The default snapshot endpoint is `/cgi-bin/CGIProxy.fcgi?cmd=snapPicture2&usr=...&pwd=...`; verify the endpoint on the camera before deployment.
