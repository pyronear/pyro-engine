...
* `ctronics` adapter, for CTronics cameras exposing an authenticated HTTP snapshot endpoint and optional ONVIF PTZ/focus
...
If your camera is not covered by the adapters above, see the [camera adapter guide](pyro_camera_api/pyro_camera_api/camera/adapters/README.md). It explains what an adapter is responsible for, how to tell whether a generic adapter already fits, and how to write a new one.
...
For CTronics, use `adapter: "ctronics"` with `ip_address` (or the configuration key), optional `username`/`password`, and optionally `port`, `protocol`, `snapshot_path`, `snapshot_command`, and `timeout`. Set `type` to `"ptz"` to enable ONVIF PTZ and focus controls; `onvif_port` defaults to `8080` and can be configured per camera. `onvif_wsdl_dir`, `onvif_profile_token`, `focus_min`, and `focus_max` are also supported for ONVIF setup and focus calibration. The tested default snapshot endpoint is `/tmpfs/snap.jpg?usr=...&pwd=...`; verify the endpoint on other models before deployment.

For the full list of adapters with their required fields, and for adding support for a camera none of them handles, see the [camera adapter guide](pyro_camera_api/pyro_camera_api/camera/adapters/README.md).
