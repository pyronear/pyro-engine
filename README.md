...
For CTronics, use `adapter: "ctronics"` with `ip_address` (or the configuration key), optional `username`/`password`, and optionally `port`, `protocol`, `snapshot_path`, `snapshot_command`, `model`, and `rtsp_path`. Set `type` to `"ptz"` to enable ONVIF controls; `onvif_port` defaults to `8080` and can be configured per camera. The tested default snapshot endpoint is `/tmpfs/snap.jpg?usr=...&pwd=...`; verify the endpoint on other models before deployment.
