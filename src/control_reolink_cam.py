...
def main():
    ...
    parser.add_argument(
        "action",
        choices=["capture", "move_camera", "get_ptz_preset", "set_ptz_preset"],
    ...
    parser.add_argument("--type", required=True, help="Type of the camera")
    ...
    if args.action == "capture":
        ...
        if args.operation:
            camera_controller.move_camera(operation=args.operation, speed=args.speed, id=args.pos_id)
        else:
        ...
        if args.pos_id is not None:
            camera_controller.set_ptz_preset(id=args.pos_id)
        else:
