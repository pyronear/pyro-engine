...
parser = argparse.ArgumentParser(description="End-to-end relay test for main Pi")
parser.add_argument(
    "--trigger",
    choices=["h", "l"],
    default="h",
...
def test_relay(name: str, gpio_pin: int, target_ip: str) -> bool:
    ...
    print(f"[CUT] GPIO {gpio_pin} -> ACTIVE for {POWER_OFF_TIME}s", flush=True)
    GPIO.output(gpio_pin, RELAY_ACTIVE)
    cut_start = time.time()
    try:
        ...
        print(f"[RESTORE] GPIO {gpio_pin} -> INACTIVE", flush=True)
        GPIO.output(gpio_pin, RELAY_INACTIVE)
